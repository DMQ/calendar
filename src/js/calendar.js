...
;(function (global, factory) {
	...
	Calendar.prototype = $.extend({}, Widget.prototype, {
		...
		_goPreOrNextMon: function (type, date) {
			var year = this.get('year'), month = +this.get('month');
		...
		tplCalendarWrap: function(){
			...
			html+=	'<span class="pre-mon"></span>'
				+ 	'<span class="title-wrap">'
				+		'<span class="year">' + this.get('year') + '</span> 年 '
				+		'<span class="month">' + this.get('month') + '</span> 月 '
				+		'<span class="head-text">' + this.get('headText') + '</span>'
				+	'</span>'
				+	'<span class="next-mon"></span>'
				+ '</div>'
				+ '<div class="days">'
		...
		tplCalendar: function (year, month, currDate) {
			...
			for (i = 0, len = preMonthDates; i < len; i++) {
				...
				$date = $( '<span class="pre" data-date="' + date + '" data-index="' + i + '">'
						  +	'<span>' + date + '</span>'
						  +'</span>');
			...
			for (i = lastDate + preMonthDates, len = datesArr.length; i < len; i++) {
				...
				$date = $( '<span class="next" data-date="' + date + '" data-index="' + i + '">'
						  +	'<span>' + date + '</span>'
						  +'</span>');
		...
		_tplCurrMonthItem: function (i, date, year, month, currDate, tagObj) {
			var $date = $( '<span class="date" data-date="' + date + '" data-index="' + i + '">'
						  +	'<span>' + date + '</span>'
						  +'</span>');
